...
if (empty($user['photo'])) {
    ...
    $userphoto = Html::img("/images/user/user_{$user['id']}/50_50_{$user['photo']}", ['class' => 'img-responsive', 'alt' => Html::encode(($user['name']) ? $user['name'] : $user['username']), 'title' => Html::encode(($user['name']) ? $user['name'] : $user['username'])]);
}
?>
<li>
